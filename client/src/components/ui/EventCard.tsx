  ...
  return (
    <div className="theme-card theme-card-hover rounded-lg p-6 h-64 flex flex-col">
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="space-y-2 text-sm text-theme-muted flex-1">
          {event.store ? (
            <>
            ...
            <div className="text-theme-muted italic">Store information not available</div>
          )}
          
          {/* Cost and Prizing Information */}
          {(event.cost || event.min_prizing || event.max_prizing) && (
            <div className="pt-2 border-t" style={{ borderColor: "var(--theme-border-soft)" }}>
              {event.cost && (
              ...
              {(event.min_prizing || event.max_prizing) && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-theme-muted truncate" title={
                    event.min_prizing && event.max_prizing ? 
                      `${event.min_prizing} - ${event.max_prizing}` :
