...
export default function StoreCard({ store, variant = 'default', basePath = '/events' }: StoreCardProps) {
  ...
  return (
    <div className={`rounded-lg border ${isLanding ? 'bg-white/60' : 'bg-white'} shadow-sm hover:shadow-md transition-shadow p-6`}>
      <h3 className={`font-semibold ${isLanding ? 'text-lg' : 'text-xl text-gray-900'}`}>
        {store.name}
      </h3>
      
      <div className={`mt-3 ${isLanding ? 'text-sm text-gray-700' : 'text-gray-700'}`}>
        {/* Location */}
        ...
        <div className="mt-3 mb-3">
          <Link
            ...
            className="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700 transition-colors w-full justify-center"
          >
